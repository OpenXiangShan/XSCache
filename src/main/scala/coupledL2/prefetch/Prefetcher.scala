...
class Prefetcher(implicit p: Parameters) extends PrefetchModule {
  ...
  if (hasBOP) {
    ...
    vbop.get.io.train.valid := train.valid && (train.bits.reqsource =/= MemReqSource.CPUStoreData.id.U)
    ...
    pbop.get.io.train.valid := train.valid && (train.bits.reqsource =/= MemReqSource.CPUStoreData.id.U)
