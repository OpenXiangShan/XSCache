...
class SlcPrefetchReq(implicit p: Parameters) extends PrefetchBundle {
...
class SlcPfEntry(implicit p: Parameters) extends PrefetchModule with HasCHIOpcodes {
  val io = IO(new Bundle {
    val alloc = Flipped(DecoupledIO(new SlcPrefetchReq))
  ...
  val req = Reg(new SlcPrefetchReq)
  ...
  assert(!io.rxrsp.valid || io.rxrsp.bits.opcode =/= RetryAck, "SLC prefetch does not support CHI retry")
...
class SlcPrefetcher(implicit p: Parameters) extends PrefetchModule with HasCHIOpcodes {
  private val slcPrefetchIdBits = TXNID_WIDTH - bankBits - 2

  require(inflightEntries > 0)
  require(slcPrefetchIdBits > 0)
  require(inflightEntries <= (1 << slcPrefetchIdBits))
  ...
  val slcPfEntries = Seq.tabulate(inflightEntries) { i =>
    val entry = Module(new SlcPfEntry)
    entry.io.id := i.U
    entry
  }

  val allocReadys = VecInit(slcPfEntries.map(_.io.alloc.ready))
  val allocOH = PriorityEncoderOH(allocReadys)
  l3PftQueue.io.deq.ready := allocReadys.asUInt.orR
  slcPfEntries.zipWithIndex.foreach { case (entry, i) =>
  ...
  val rxrspHit = VecInit(slcPfEntries.zipWithIndex.map {
    case (entry, i) => entry.io.waitResp && io.rxrsp.bits.txnID === i.U
  }).asUInt.orR
  io.rxrsp.ready := rxrspHit
  assert(!io.rxrsp.valid || rxrspHit, "SLC prefetch received a response with no matching entry")

  fastArb(slcPfEntries.map(_.io.txreq), io.txreq, Some("slc_prefetch_txreq"))
...
class Prefetcher(implicit p: Parameters) extends PrefetchModule {
  ...
  val banks = 1 << bankBits
  val slcPrefetcher = Module(new SlcPrefetcher)
  slcPrefetcher.io.recv := io.l3_recv
  io.slc_txreq <> slcPrefetcher.io.txreq
  slcPrefetcher.io.rxrsp <> io.slc_rxrsp
