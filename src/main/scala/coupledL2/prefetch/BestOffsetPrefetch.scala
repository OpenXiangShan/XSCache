...
class VBestOffsetPrefetch(implicit p: Parameters) extends BOPModule {
  ...
  // BOP trains on store requests (the offset table should keep learning), but
  // store prefetch requests are generated by the L1 store buffer instead, so no
  // prefetch request is sent out for a store train
  val s0_isStoreTrain = io.train.bits.reqsource === MemReqSource.CPUStoreData.id.U
  ...
  when(s0_fire) {
    if(virtualTrain) s1_req_valid := enable && !prefetchDisable && io.pbopCrossPage && !s0_isStoreTrain // now pbopCrossPage is true.B by default
    else s1_req_valid := enable && !prefetchDisable && !s0_crossPage && !s0_isStoreTrain // stop prefetch when prefetch req crosses pages
  }.elsewhen(s1_fire){
  ...
  XSPerfAccumulate("bop_drop_for_store_train", scoreTable.io.req.fire && s0_isStoreTrain)
}
...
class PBestOffsetPrefetch(implicit p: Parameters) extends BOPModule {
  ...
  // store requests only train the offset table, no prefetch request is sent
  val isStoreTrain = io.train.bits.reqsource === MemReqSource.CPUStoreData.id.U
  when(io.req.fire) {
    req_valid := false.B
  }
  when(scoreTable.io.req.fire) {
    ...
    req_valid := enable && !crossPage && !prefetchDisable && !isStoreTrain // stop prefetch when prefetch req crosses pages
  }
  ...
  XSPerfAccumulate("bop_drop_for_store_train", scoreTable.io.req.fire && isStoreTrain)
}
