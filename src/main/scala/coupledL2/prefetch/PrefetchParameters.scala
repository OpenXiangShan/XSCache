...
object PfSource extends Enumeration {
  ...
  val CDP     = Value("CDP")
  // Store prefetch is generated by the upper-level Sbuffer path. It is tracked
  // in metadata for accuracy/late attribution but is not an L2 prefetcher
  // controlled by PrefetchController.
  ...
  def fromMemReqSource(s: UInt): UInt = {
    ...
    switch(s) {
      ...
      is (MemReqSource.Prefetch2L2Store.id.U) { pfsrc := Store.id.U }
    }
    pfsrc
  }
}
