 ...
 * [1] Robert Cooksey, Stephan Jourdan, and Dirk Grunwald. "[A stateless, content-directed data prefetching 
 * mechanism.](https://doi.org/10.1145/635508.605427)" ACM SIGOPS Operating System Review. 2002
 * *************************************************************************************
 */
...
trait HasCDPParams extends HasPrefetcherHelper with HasCoupledL2Parameters {
  ...
  require(depthThreshold <= cdpPfDepthMax.get, s"depthThreshold(${depthThreshold}) should be less than pfDepthMax(${cdpPfDepthMax.get})")
...
class CDPDetectTask(val dataBits: Int)(implicit p: Parameters) extends CDPBundle {
  ...
  val pfDepth = UInt(cdpPfDepthBits.get.W)
...
class DetectPipeline(name: String)(implicit p: Parameters) extends CDPModule {
  ...
  val s3_depth = RegNext(Mux(
    ...
    Mux(s2_depth === 0.U, cdpPfDepthMax.get.U, s2_depth + 1.U)
  ))
...
class PrefetchFilterEntry(implicit p: Parameters) extends CDPBundle {
  ...
  def toPrefetchReq(): PrefetchReq = {
    ...
    req.cdpPfDepth.foreach(_ := pfDepth)
    ...
    val hitTrigger = detectTrig.bits.isHit  &&
      (
        if (useFilteredDetect) {
          detectTrigFromCDP &&
            (detectTrig.bits.pfDepth === 1.U || detectTrig.bits.pfDepth === cdpPfDepthMax.get.U) ||
