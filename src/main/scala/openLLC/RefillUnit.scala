...
class RefillUnit(implicit p: Parameters) extends LLCModule with HasCHIOpcodes {
  ...
  def updateRespDataEntry(entry: RefillEntry, rspData: RespWithData): Unit = {
    ...
    entry.data.data(beatId) := rspData.data
    entry.task.resp := rspData.resp
    when(rspData.opcode === SnpRespData) {
      val src_idOH  = UIntToOH(rspData.srcID)(numRNs - 1, 0)
  ...
  val respDataUpdateVec = VecInit(buffer.map(e => e.task.reqID === io.respData.bits.txnID && e.valid))
  val snRespDataUpdateVec = VecInit(buffer.map(e =>
    e.task.reqID === io.snRespData.bits.txnID && e.valid && e.task.chiOpcode === StashOnceShared
  ))
  assert(!io.respData.valid || PopCount(respDataUpdateVec) < 2.U, "Refill task repeated")
  assert(!io.snRespData.valid || PopCount(snRespDataUpdateVec) < 2.U, "Refill task repeated")
  buffer.zipWithIndex.foreach { case (entry, i) =>
    val respDataHit = io.respData.valid && respDataUpdateVec(i)
    val snRespDataHit = io.snRespData.valid && snRespDataUpdateVec(i)
    assert(!(respDataHit && snRespDataHit), "Refill task receives data from multiple sources")
    when(respDataHit) {
      updateRespDataEntry(entry, io.respData.bits)
    }.elsewhen(snRespDataHit) {
      updateRespDataEntry(entry, io.snRespData.bits)
    }
  }
  ...
  when(io.respData.valid && rsp.valid) {
    when(io.respData.bits.opcode === SnpRespData && rsp.bits.opcode === SnpResp) {
      when(io.respData.bits.txnID === rsp.bits.txnID) {
        ...
        val canUpdate = Cat(update_vec).orR
        when(canUpdate) {
          val src_idOH_dat = UIntToOH(io.respData.bits.srcID)(numRNs - 1, 0)
